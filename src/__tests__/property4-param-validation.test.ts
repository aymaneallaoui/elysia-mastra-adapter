        ...
        async (invalidId) => {
          ...
          const route = {
            ...
            pathParamSchema: z.object({
              id: z.string().regex(/^\d+$/, 'ID must be numeric').transform(Number).pipe(z.number().positive('ID must be positive')),
            }),
          };
        ...
        async (invalidValue) => {
          ...
          const route = {
            ...
            queryParamSchema: z.object({
              limit: z.string().regex(/^\d+$/, 'Limit must be numeric').transform(Number).pipe(z.number().min(1).max(100)),
            }),
          };
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (validUser) => {
          ...
          const body = await response.json() as { created: boolean; user: { name: string; email: string } };
