...
describe('Property 2: Context Middleware Attachment', () => {
  ...
  const toolsRecordArb = fc.array(toolNameArb, { minLength: 0, maxLength: 5 }).map((names) => {
  ...
  const requestContextArb = fc
    .array(
      fc.tuple(fc.stringMatching(/^[a-z][a-z0-9_]{0,9}$/), fc.oneof(fc.string(), fc.integer())),
  ...
  test('context middleware attaches mastra instance to all requests', async () => {
    await fc.assert(
      fc.asyncProperty(toolsRecordArb, async (tools) => {
  ...
  test('context middleware attaches RequestContext map to all requests', async () => {
    await fc.assert(
      fc.asyncProperty(requestContextArb, async (paramsContext) => {
        const app = new Elysia();
        const mastra = createMockMastra();
  ...
  test('all context properties are attached together for any request', async () => {
    await fc.assert(
      fc.asyncProperty(toolsRecordArb, requestContextArb, async (tools, paramsContext) => {
        const app = new Elysia();
        const mastra = createMockMastra();
