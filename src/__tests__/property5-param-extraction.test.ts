...
describe('Property 5: Parameter Extraction', () => {
  ...
  test('getParams extracts URL path parameters correctly', async () => {
    await fc.assert(
      fc.asyncProperty(urlParamsArb, async (expectedUrlParams) => {
        ...
        app.get(pathPattern, async (context) => {
          const route = { path: pathPattern, method: 'GET' as const } as Parameters<
            typeof server.getParams
          >[0];
  ...
  test('getParams extracts query parameters correctly', async () => {
    await fc.assert(
      fc.asyncProperty(queryParamsArb, async (expectedQueryParams) => {
        const app = new Elysia();
        const mastra = createMockMastra();
        ...
        app.get('/test', async (context) => {
          const route = { path: '/test', method: 'GET' as const } as Parameters<
            typeof server.getParams
          >[0];
          const params = await server.getParams(route, context);
          extractedParams = params;
  ...
  test('getParams extracts request body correctly', async () => {
    await fc.assert(
      fc.asyncProperty(bodyArb, async (expectedBody) => {
        const app = new Elysia();
        const mastra = createMockMastra();
        ...
        app.post('/test', async (context) => {
          const route = { path: '/test', method: 'POST' as const } as Parameters<
            typeof server.getParams
          >[0];
          const params = await server.getParams(route, context);
          extractedParams = params;
